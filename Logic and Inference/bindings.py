...
def match(statement1, statement2, bndngs = None):
    if bndngs == None:
        bndngs = bindings()
    if len(statement1) != len(statement2):
        return False
    if len(statement1) == 0:
        return bndngs.bindings
    if varq(statement2[0]):
        if not bndngs.test_and_bind(statement2[0], statement1[0]):
            return False
    elif statement1[0] != statement2[0]:
        return False
    return match(statement1[1:], statement2[1:], bndngs)
